...
function initializeApp() {
    ...
    // Update saved recipes badge
    updateSavedRecipeCount();
    
    // Add popular categories to footer
...
function setupEventListeners() {
    ...
    // Add CSS for menu-open state to prevent background scrolling
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function (e) {
            if (this.getAttribute('href') !== '#') {
                ...
                // Handle special case for saved recipes link
...
async function handleSearch() {
    ...
    try {
        // For TheMealDB API, we need to try different search approaches
        let recipes = await fetchRecipesByName(query);
        
        // If no results by name, try searching by first letter
...
async function fetchRecipesByName(query) {
    try {
        console.log(`Searching for recipes with name: ${query}`);
...
async function fetchRecipesByFirstLetter(letter) {
    try {
        console.log(`Searching for recipes starting with: ${letter}`);
        ...
        const data = await response.json();
        console.log('API First Letter Response:', data);
...
// Replace the existing fetchRecipes function with this one
...
function displayRecipes(recipes) {
    ...
    recipes.forEach(recipe => {
        ...
        // Add class to animate as soon as cards are created
        setTimeout(() => {
...
function toggleBookmark(id, button, recipe) {
    ...
    // Update saved recipes count badge
    updateSavedRecipeCount();
    
    // Refresh saved recipes view if it's visible
...
function updateSavedRecipesView() {
    ...
    // Sort saved recipes by date added (newest first)
...
async function loadFeaturedRecipes() {
    try {
        // Let's get some random recipes for the featured section
        const response = await fetch(`${API_BASE_URL}/randomselection.php`);
        const data = await response.json();
        
        if (!data.meals) {
            // If the random endpoint doesn't work, fallback to some specific categories
        ...
        // Fallback to hardcoded recipes if all API calls fail
...
// Display featured recipes in carousel
...
function moveCarousel(direction) {
    ...
    // Handle mobile layout - make sure we don't slide too far
    ...
    if (direction === 'prev') {
        ...
        // Move to next slide, account for number of items and visible items
...
function displayCategories(categories) {
    ...
    categories.forEach(category => {
        ...
        categoryCard.addEventListener('click', () => {
            // Set the category as search term
...
function populateFooterCategories(categories) {
    ...
    // If categories are not provided, try to fetch them
    if (!categories) {
...
async function showRecipeDetails(id) {
    ...
    try {
        ...
        // Format instructions with better steps
        ...
        // Add bookmark functionality in modal
        ...
        bookmarkBtn.addEventListener('click', () => {
            ...
            // Update any matching recipe cards in the UI
...
function showLoading() {
    ...
    // Clear any existing results while loading
    recipeContainer.innerHTML = '';
}
